...
export default function Topbar({ isAuthenticated, user, sidebarOpen, onToggleSidebar }: TopbarProps) {
  ...
  const handleLanguageSelect = (language: "tigrinya" | "amharic" | "english") => {
    setSelectedLanguage(language)
    setIsLanguageModalOpen(false)
    // You might want to save the language preference to localStorage or context here
  ...
  return (
    <>
      <div className="flex items-center justify-between border-b border-border bg-background px-4 py-3 md:px-6 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="text-lg font-bold text-foreground hidden sm:block hover:text-primary transition-colors"
          >
            LEGNA
          </button>
        </div>
      ...
      {/* Render LanguageModal when open */}
      {isLanguageModalOpen && (
